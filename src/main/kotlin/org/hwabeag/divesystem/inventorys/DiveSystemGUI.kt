...
class DiveSystemGUI(player: Player) {
    ...
    init {
        ...
        inv = Bukkit.createInventory(null, line * 9 + 9, "${playerShop}잠수상점")
        initItemSetting(player)
    }

    private fun getDiveShopItemData(player: Player, key: Int): ItemStack {
        val name = player.name
        val page = playerConfig.getInt("$name.페이지")
        val shopName = playerConfig.getString("$name.구매상점") ?: return ItemStack(Material.BARRIER)
        ...
        lore.add(ChatColor.translateAlternateColorCodes('&', "&f&l- 우클릭시 구매합니다."))
        meta.lore = lore
        clone.itemMeta = meta
        return clone
    }
    ...
    private fun initItemSetting(player: Player) {
        val name = player.name
        val page = playerConfig.getInt("$name.페이지")
        val playerShop = playerConfig.getString("$name.구매상점") ?: return
        val line = diveSystemConfig.getInt("잠수상점.$playerShop.라인")
        val section = diveSystemConfig.getConfigurationSection("잠수상점.$playerShop.물품.$page")
        section?.getKeys(false)?.forEach { key ->
            inv.setItem(key.toInt(), getDiveShopItemData(player, key.toInt()))
        }
